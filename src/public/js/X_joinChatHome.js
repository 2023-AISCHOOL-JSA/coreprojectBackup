...
const handleRoomSubmit = (event) => {
  ...
  chatSocket.emit("enter_room", { room_name: room_name, nickname: nickname });
  chatSocket.emit("welcome", { room_name: room_name, nickname: nickname });
  const newUrl = `${window.location.origin}/chat/chatroom/${encodeURIComponent(
    room_name
  )}`;
  window.location.href = newUrl;
};

// chatSocket.on("room_info", ({ room_name }) => {
//   // 채팅방으로 이동
...
const handleNicknameSubmit = (event) => {
  ...
  chatSocket.emit("nickname", { nickname }, () => {
...
chatSocket.on("room_change", ({ publicRooms }) => {
